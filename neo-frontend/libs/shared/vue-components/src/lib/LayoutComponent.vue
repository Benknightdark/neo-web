...
const navigateTo = (e: MouseEvent, path: string) => {
  if (router) {
    try {
      ...
      // Ensure we match a valid registered route and not a wildcard fallback (like /:pathMatch(.*)*)
      const hasRoute = resolved && resolved.matched.length > 0 && 
                       !resolved.matched.some(m => m.name === 'NotFound' || m.path.includes('*') || m.path.includes(':'));
      if (hasRoute) {
        e.preventDefault();
        router.push(path);
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
          ...
          <div class="flex items-center space-x-10">
            <a href="/home/" @click="navigateTo($event, '/home/')" class="flex items-center space-x-2 group">
            ...
            <nav class="hidden md:flex space-x-6">
              <a href="/home/" @click="navigateTo($event, '/home/')" class="text-sm font-semibold text-slate-600 hover:text-indigo-600 hover:-translate-y-0.5 transition-all duration-200">
                首頁 (Home)
              </a>
              <a href="/introduction/" @click="navigateTo($event, '/introduction/')" class="text-sm font-semibold text-slate-600 hover:text-indigo-600 hover:-translate-y-0.5 transition-all duration-200">
                介紹 (Introduction)
              </a>
              <a href="/privacy/" @click="navigateTo($event, '/privacy/')" class="text-sm font-semibold text-slate-600 hover:text-indigo-600 hover:-translate-y-0.5 transition-all duration-200">
                隱私政策 (Privacy)
              </a>
            </nav>
          </div>
      ...
      >
        <div v-show="isMobileMenuOpen" class="md:hidden bg-white border-b border-slate-200" id="mobile-menu">
          <div class="px-2 pt-2 pb-4 space-y-1">
            <a href="/home/" @click="navigateTo($event, '/home/')" class="block px-3 py-2 rounded-md text-base font-semibold text-slate-700 hover:text-indigo-600 hover:bg-slate-50 transition-colors">
              首頁 (Home)
            </a>
            <a href="/introduction/" @click="navigateTo($event, '/introduction/')" class="block px-3 py-2 rounded-md text-base font-semibold text-slate-700 hover:text-indigo-600 hover:bg-slate-50 transition-colors">
              介紹 (Introduction)
            </a>
            <a href="/privacy/" @click="navigateTo($event, '/privacy/')" class="block px-3 py-2 rounded-md text-base font-semibold text-slate-700 hover:text-indigo-600 hover:bg-slate-50 transition-colors">
              隱私政策 (Privacy)
            </a>
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0 text-slate-500 text-sm">
          ...
          <div class="flex space-x-6">
            <a href="/privacy/" @click="navigateTo($event, '/privacy/')" class="hover:text-indigo-600 hover:underline transition-all duration-200">
              隱私政策 (Privacy)
            </a>
          </div>
        </div>
