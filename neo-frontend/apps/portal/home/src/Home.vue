...
const testApiConnection = async () => {
  ...
  try {
    ...
    apiStatus.value = 'error';
    errorMessage.value = errorObject.message || '連線失敗，請檢查 API 是否已啟動';
  }
};
...
<template>
  <div
    class="min-h-screen bg-gradient-to-tr from-slate-50 via-blue-50 to-indigo-100 dark:from-slate-950 dark:via-slate-900 dark:to-indigo-950 text-slate-800 dark:text-slate-100 transition-colors duration-300 py-12 px-6"
  >
    <div class="max-w-4xl mx-auto flex flex-col justify-between min-h-[80vh]">
      <!-- Hero Header -->
      <header class="text-center mb-12">
        <div
          class="w-16 h-16 rounded-2xl bg-gradient-to-tr from-blue-600 to-indigo-600 flex items-center justify-center text-white font-bold text-2xl shadow-xl shadow-blue-500/30 mx-auto mb-6"
        >
          N
        </div>
        <span
          class="px-4 py-1.5 rounded-full text-xs font-semibold tracking-wider text-blue-600 bg-blue-500/10 dark:text-blue-400 dark:bg-blue-500/5 border border-blue-500/20 uppercase inline-block mb-4"
        >
          全新系統入口
        </span>
        <h1 class="text-4xl md:text-5xl font-black tracking-tight mb-4">
          歡迎來到
          <span
            class="bg-clip-text text-transparent bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-500"
            >Neo Web Portal</span
          >
        </h1>
      ...
      <section class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
        ...
        <router-link
          ...
          :class="
            isHoveredIntro
              ? 'border-blue-500/50 shadow-xl shadow-blue-500/5 -translate-y-1'
              : 'border-white/20 dark:border-slate-800 shadow-md'
          "
        ...
        >
          <div
            ...
            :class="{ 'scale-110': isHoveredIntro }"
          >
            🚀
          </div>
          ...
          <div class="mt-6 flex items-center text-sm font-semibold text-blue-500">
            進入模組
            <span class="ml-1 transition-transform" :class="{ 'translate-x-1': isHoveredIntro }"
              >→</span
            >
          </div>
        </router-link>
        ...
        <router-link
          ...
          :class="
            isHoveredPrivacy
              ? 'border-indigo-500/50 shadow-xl shadow-indigo-500/5 -translate-y-1'
              : 'border-white/20 dark:border-slate-800 shadow-md'
          "
        ...
        >
          <div
            class="text-4xl mb-4 transform transition-transform duration-300"
            :class="{ 'scale-110': isHoveredPrivacy }"
          >
            🛡️
          </div>
          ...
          <div class="mt-6 flex items-center text-sm font-semibold text-indigo-500">
            進入模組
            <span class="ml-1 transition-transform" :class="{ 'translate-x-1': isHoveredPrivacy }"
              >→</span
            >
          </div>
        </router-link>
      ...
      <section
        class="bg-white/70 dark:bg-slate-800/40 backdrop-blur-md p-8 rounded-3xl border border-white/20 dark:border-slate-800 shadow-lg mb-8"
      >
      ...
      <!-- Footer -->
      <footer
        class="mt-8 pt-6 border-t border-slate-200/50 dark:border-slate-800/50 text-center text-xs text-slate-400 dark:text-slate-500 flex flex-col md:flex-row justify-between items-center gap-4"
      >
        <p>© 2026 Neo Web. 模組化前端入口建置完成。</p>
        <div class="flex gap-4">
          <router-link to="/introduction/" class="hover:text-blue-500">專案文件</router-link>
          <router-link to="/privacy/" class="hover:text-blue-500">隱私條款</router-link>
        </div>
      </footer>
    </div>
  </div>
